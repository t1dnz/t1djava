...
data class T1DInputs (
    val insulinBoluses: SortedSet<BolusInsulin> = sortedSetOf(dataComparitor),
    val insulinBasalChanges: SortedSet<BasalInsulinChange> = sortedSetOf(dataComparitor),
    val bglReadings: SortedSet<BGLReading> = sortedSetOf(dataComparitor),
    val carbs: SortedSet<CarbIntake> = sortedSetOf(dataComparitor),
)

// This is the grouoping of models used to calculate the various states of a t1d body, e.g. insulin on board...
class T1DSimulation {
    ...
    fun addPatientData(patientData: T1DInputs) {
    ...
    private fun processBasalChanges(midnightMinus: LocalDateTime) {
        ...
        while (basalTime > midnightMinus) {
            ...
            if (isEmpty) {
                Log.info("processBasalChanges no finished at time $basalTime")
                break
            }
